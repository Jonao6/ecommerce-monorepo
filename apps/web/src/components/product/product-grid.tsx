...
const ProductImage = ({ src, alt, className }: ProductImageProps) => {
	return (
		...
		>
			<Image src={src} fill alt={alt} className="object-cover" priority={true} sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
		</div>
	);
};
...
const ProductItem = ({ product }: ProductItemProps) => {
	return (
		...
		>
			<div >
				<ProductImage src={product.image} alt={product.name} className='h-72'/>
...
const ProductGrid = ({ products, loading }: ProductGridProps) => {
	...
	return (
		<div className="grid grid-cols-2 grid-rows-3 md:grid-cols-3 md:grid-rows-2 gap-10 px-40 py-10 w-full h-full">
